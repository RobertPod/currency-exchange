...
public class RatesProvider {
    ...
    public Map<DateTime, Double> getExchangeRateList(String base, Currency requested, DateTime startAt,
        DateTime endAt) {
        List<ExchangeRates> exchangeRateses;

        try {
            exchangeRateses = apiClient.getHistoricalRates(startAt, endAt, base);
        } catch (IllegalArgumentException e) {
